    ...
    public class CapNhatPhieuXuatHangViewModel: BaseViewModel
    {
        ...
        public CapNhatPhieuXuatHangViewModel(PhieuXuatHangHienThi phieuXuatHangHienThi)
        {
            ...
            XoaCommand = new RelayCommand<Window>((p) => { return true; }, (p) => {
                var result = MessageBox.Show("Bạn có muốn xóa phiếu xuất hàng này không?", "System Warning", MessageBoxButton.YesNo, MessageBoxImage.Warning);

                if (result == MessageBoxResult.Yes)
                {
                    using (DBQuanLyCacDaiLyEntities db = new DBQuanLyCacDaiLyEntities())
                    {
                        foreach (var i in db.ChiTietPhieuXuatHangs)
                        {
                            if (i.IdPhieuXuatHang == PhieuXuatHangHienThi.PhieuDaiLy.Id)
                            {
                                db.ChiTietPhieuXuatHangs.Remove(i);
                            }
                        }

                        db.PhieuXuatHangs.Remove(db.PhieuXuatHangs.Where(i => i.IdPhieuDaiLy == PhieuXuatHangHienThi.PhieuXuatHang.IdPhieuDaiLy).FirstOrDefault());

                        db.PhieuDaiLies.Remove(db.PhieuDaiLies.Where(i => i.Id == PhieuXuatHangHienThi.PhieuDaiLy.Id).FirstOrDefault());
            ...
            LuuThayDoiCommand = new RelayCommand<Window>((p) => { return true; }, (p) =>
            {
                ...
                if (result == MessageBoxResult.Yes)
                {
                    PhieuXuatHangHienThi.TongTien = TongTien;
                    PhieuXuatHangHienThi.DaiLy = DaiLy;
                    PhieuXuatHangHienThi.PhieuDaiLy.IdDaiLy = DaiLy.Id;
                    PhieuXuatHangHienThi.PhieuXuatHang.TongTien = tongtien;

                    using(DBQuanLyCacDaiLyEntities db = new DBQuanLyCacDaiLyEntities())
                    {
                        var phieuXuatHang = db.PhieuXuatHangs.Where(pxh => pxh.PhieuDaiLy.Id == PhieuXuatHangHienThi.PhieuDaiLy.Id).FirstOrDefault();

                        phieuXuatHang.PhieuDaiLy.IdDaiLy = DaiLy.Id;
                        ...
                        {
                            db.ChiTietPhieuXuatHangs.Add(new ChiTietPhieuXuatHang
                            {
            ...
            AddCommand = new RelayCommand<CapNhatPhieuXuatHangWindow>((p) => { return true; }, (p) => {
            ...
            DeleteCommand = new RelayCommand<CapNhatPhieuXuatHangWindow>((p) => { return true; }, (p) => {
                tongtien -= SanPhamXuat.ThanhTienNum;
                TongTien = ConvertNumber.convertNumberDecimalToString(tongtien);
            ...
            EditCommand = new RelayCommand<CapNhatPhieuXuatHangWindow>((p) => { return true; }, (p) => {
                int soLuongXuat = ConvertNumber.convertStringtoInt(SoLuong);
        ...
        {
            string filePath = "";
            string ngayLapPhieu = PhieuXuatHangHienThi.PhieuXuatHang.NgayLapPhieu.ToString("yyyy-MM-dd");
            ...
            try
            {
                ...
                using (ExcelPackage ep = new ExcelPackage())
                {
                    ...
                    ws.Cells[2, 6].Value = PhieuXuatHangHienThi.PhieuXuatHang.NgayLapPhieu.ToShortDateString();
